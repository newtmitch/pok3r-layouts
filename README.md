...
Note that these colors and references are done on a Pok3r RGB - if you're not using a Pok3R RGB, the colors for
the layers will be different.

The following layers are used in either OS:
* Windows: Layer 1 (no LED), `FN + M`
* Mac: Layer 2 (red LED), switch to it with: `FN + ,`
* Mac: Layer 3 (blue LED), switch with: `FN + .`
...
### Disable caps lock
Turn on dipswitch 3 to disable caps lock. Who uses that thing anyway?
